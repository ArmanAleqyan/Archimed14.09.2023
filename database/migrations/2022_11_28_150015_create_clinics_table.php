<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('clinics', function (Blueprint $table) {
            $table->id();
            $table->string('clinic_id')->nullable();
            $table->string('title')->nullable();
            $table->string('fmOrgId')->nullable();
            $table->string('typeId')->nullable();
            $table->string('ageGroup')->nullable();
            $table->string('name')->nullable();
            $table->string('code')->nullable();
            $table->string('address')->nullable();
            $table->string('coordinates')->nullable();
            $table->string('phone')->nullable();
            $table->string('email')->nullable();
            $table->string('workTime')->nullable();
            $table->string('timeOffset')->nullable();
            $table->string('description')->nullable();
            $table->string('brandId')->nullable();
            $table->string('nearbyTransportHubs')->nullable();
            $table->string('isAnonymousVisitsProhibited')->nullable();
            $table->string('imageCount')->nullable();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('clinics');
    }
};
